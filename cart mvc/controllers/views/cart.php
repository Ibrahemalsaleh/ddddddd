...
<body>
    ...
    <div class="container">
        <h1>Shopping Cart</h1> <?php if (empty($cartItems)): ?>
            <p>Your cart is empty.</p> <?php else: ?>
            <form action="?page=update_cart_quantity" method="post">
                <ul class="list-group mb-3">
                    <?php foreach ($cartItems as $item): ?>
                        <li class="list-group-item d-flex justify-content-between lh-sm">
                            <div>
                                <h6 class="my-0"><?php echo htmlspecialchars($item['name']); ?></h6>
                                <small class="text-muted">Price: <?php echo htmlspecialchars(number_format($item['price'], 2)); ?>$</small> </div>
                            <div class="text-end">
                                <input type="number" name="quantities[<?php echo htmlspecialchars($item['product_id']); ?>]"
                                       value="<?php echo htmlspecialchars($item['quantity']); ?>" min="0" class="form-control form-control-sm quantity-input" style="width: 70px;">
                                <span class="text-muted"><?php echo htmlspecialchars(number_format($item['price'] * $item['quantity'], 2)); ?>$</span>
                                <a href="?page=remove_cart_item&product_id=<?php echo htmlspecialchars($item['product_id']); ?>" class="btn btn-danger btn-sm">Remove</a> </div>
                        </li>
                    <?php endforeach; ?>
                ...
                <button type="submit" class="btn btn-primary">Update Quantities</button> <a href="?page=checkout" class="btn btn-success">Checkout</a> <a href="?page=clear_cart" class="btn btn-danger">Clear Cart</a> </form>
        <?php endif; ?>
    </div>
